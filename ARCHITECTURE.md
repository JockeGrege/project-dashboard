...
public/404.html             # not used by HashRouter, kept only if we switch to path routing
.github/workflows/deploy.yml # build + publish dist/ to GitHub Pages on push to main
playwright/                  # one smoke: open → pick project → type → Enter → filed
```
...
### The one deferred value — `VITE_ALLOWED_UID`

No one has signed in yet, so the account UID isn't known. Handled without blocking the
build:

1. `firebase/auth.ts` logs `signed in as UID <uid>` prominently on every sign-in and, if
   `VITE_ALLOWED_UID` is unset or `__REPLACE_ME__`, shows a one-line banner with the UID
   and a copy button instead of enforcing the pin.
2. First run (local against the emulator, or the deployed site once), sign in, copy the
   UID into `.env` + the Actions secret, and paste it into `firestore.rules`.
3. From then on `auth.ts` enforces `uid === VITE_ALLOWED_UID` (sign out + error
   otherwise) and the rules reject every other UID.

Local dev uses the Emulator Suite regardless (no quota, wipeable, reseedable); the real
config is exercised only by the deployed site and the pre-deploy smoke.
